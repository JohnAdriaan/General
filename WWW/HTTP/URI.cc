...
#undef min // I don't know who defined this, but...

   Pos colon = uri.find(':');
   Pos minStart = std::min(colon, slash);
   host = uri.substr(0, minStart);
   if (colon<slash) {
      String service = uri.substr(colon+1, slash-colon);
      port = Query::Service::Find(service);
   } // if
   ...
   if (slash!=String::npos) {
      ...
      Pos hash = uri.find('#');
      Pos minEnd = std::min(questionMark, hash);
      path = uri.substr(0, minEnd);
